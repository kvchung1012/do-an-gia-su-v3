 ...
 const handleSaveDate = async (e)=>{
  ...
  return (
    ...
    >
        ...
        <Stack direction="row" spacing={6}>
          ...
          <Stack pt={2} spacing={2}>
            ...
            <Stack direction="row" spacing={1}>
              {availableDate ? (
                availableDate.filter(x=>x.date == datePick)?.map((item, i) => (
                  <Chip
                    key={i}
      ...
      {
  showForm && <TutorDateForm isOpen={showForm} onClose={()=>{setShowForm(false)}} data={undefined} onSave={handleSaveDate}/>
}
    </Card>
